...
class FormatShape(object):
    ...
    def __init__(self, input_format):
        ...
        if self.input_format not in ['NCTHW', 'NCHW', 'NCTHW_Heatmap', 'NCTHW_1DHeatmapBishal', 'NCTHW_1DHeatmapBishal_stgcn']:
            raise ValueError(
    ...
    def __call__(self, results):
        ...
        elif self.input_format == 'NCTHW_1DHeatmapBishal':
            if 'imgs' in results:
                num_clips = results['num_clips']
                clip_len = results['clip_len']
                imgs = results['imgs']
                ...
                imgs = imgs.reshape((-1, num_clips, clip_len) + imgs.shape[1:])
                # print('HEY HERE HERE: ', imgs.shape)
                # N_crops x N_clips x L x C x H x W
                imgs = np.transpose(imgs, (0, 1, 3, 2, 4))
                # N_crops x N_clips x C x L x H x W
                imgs = imgs.reshape((-1, 1, ) + imgs.shape[2:]) # I added 1 after -1, in imgs = imgs.reshape((-1, 1, )
                # M' x 1 x C x L x H + W
                # M' = N_crops x N_clips
        ...
        elif self.input_format == 'NCTHW_1DHeatmapBishal_stgcn':
            if 'imgs' in results:
                num_clips = results['num_clips']
                clip_len = results['clip_len']
                imgs = results['imgs']
                ...
                imgs = imgs.reshape((-1, num_clips, clip_len) + imgs.shape[1:])
                # N_crops x N_clips x L x C x H + W
                # imgs = np.transpose(imgs, (0, 1, 2, 3, 4))
                # N_crops x N_clips x H + W x L x C
                imgs = imgs.reshape((-1, 1, ) + imgs.shape[2:]) # I added 1 after -1, in imgs = imgs.reshape((-1, 1, )
                # M' x 1 x H + W x L x C

                # N x M x T x V x C - THis is needed for stgcn 
                # M' = N_crops x N_clips
                results['keypoint'] = imgs
                results['input_shape'] = imgs.shape
