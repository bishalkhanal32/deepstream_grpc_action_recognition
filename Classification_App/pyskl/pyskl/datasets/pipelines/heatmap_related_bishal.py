...
class GeneratePoseTargetBishal:
    ...
    def generate_a_heatmap(self, arr, centers, max_values, img_h, img_w):
        ...
        for center, max_value in zip(centers, max_values):
            ...
            patch = np.exp(-((x - mu_x)**2) / 2 / sigma**2)
            # print("x coordinate with its patch")
            # print(mu_x, sigma, patch)
            patch = patch * max_value
            arr[st_x:ed_x] = np.maximum(arr[st_x:ed_x], np.squeeze(patch))

            patch = np.exp(-((y - mu_y)**2) / 2 / sigma**2)
            patch = patch * max_value
            arr[(img_w + st_y):(img_w + ed_y)] = np.maximum(arr[(img_w + st_y):(img_w + ed_y)], np.squeeze(patch))
    ...
    def generate_heatmap(self, arr, kps, max_values, img_h, img_w):
        ...
        if self.with_kp:
            ...
            for i in range(num_kp):
                self.generate_a_heatmap(arr[i], kps[:, i], max_values[:, i], img_h, img_w)
    ...
    def gen_an_aug(self, results):
        ...
        ret = np.zeros([num_frame, num_c, img_h + img_w], dtype=np.float32) #I changed this line
        ...
        for i in range(num_frame):
            ...
            self.generate_heatmap(ret[i], kps, kpscores, img_h, img_w)
        return ret

    def __call__(self, results):
        heatmap = self.gen_an_aug(results)
        # print("HEY HEY CHECK THIS ", heatmap.shape) #(48, 17, 112)
