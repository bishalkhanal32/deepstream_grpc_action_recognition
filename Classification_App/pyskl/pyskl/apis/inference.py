...
def inference_recognizer_bishal(model, data, outputs=None, as_tensor=True, **kwargs):
    top5_label = None
    cfg = model.cfg
    ...
    with OutputHook(model, outputs=outputs, as_tensor=as_tensor) as h:
        with torch.no_grad():
            scores = model(run = False, return_loss=False, **data)[0]
            # print(scores.shape)
