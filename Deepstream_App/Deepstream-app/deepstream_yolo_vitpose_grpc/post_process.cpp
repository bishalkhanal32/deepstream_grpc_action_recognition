...
// topology for output from ViTPose
...
void normalize_peak(float &i, float &j, int height, int width)
{
  i = (i + 0.5) / height;
  j = (j + 0.5) / width;
}
...
void find_peak(Vec2D<float> &peaks, void *cmap_data, NvDsInferDims &cmap_dims, float threshold)
{
  int width = cmap_dims.d[2];
  int height = cmap_dims.d[1];

  peaks.assign(cmap_dims.d[0], Vec1D<float>(2, 0));
  ...
  for (unsigned int c = 0; c < cmap_dims.d[0]; c++)
  {
    ...
    // g_print("Max value: %f \t", max_value);

    if (max_value >= threshold)
    {
      normalize_peak(max_i, max_j, height, width);
      // g_print("max_i: %f \t", max_i);
      // g_print("max_j: %f \t", max_j);
      peaks[c][0] = max_i;
      peaks[c][1] = max_j;
    }
  }
...
// no need below function for ViTPose model
